...
#define SAMPLE_RATE             8000  // typical values: 8000 .. 44100, use e.g 8K (and 8 bit mono) for smallest .wav files  
...
#define GAIN_BOOSTER_I2S       28   // original I2S streams is VERY silent, so we added an optional GAIN booster for INMP441
...
{
  static int debug_chunk_count = 0;  // Counter to limit debug output
  ...
  { 
    ...
    debug_chunk_count = 0;  // Reset debug counter
  ...
  else if (flg_is_recording)  // here we land when recording started already -> task: append record buffer to file
  { 
    ...
    uint8_t audio_buffer_8bit[1024];    // 1024 values [1024 bytes] <- self calculated values in case BITS_PER_SAMPLE == 8
    ...
    // Debug: Print first 16 samples of each chunk (limit to first 5 chunks)
    ...
    // Save audio data to SD cardamom (appending chunk array to file end)
    File audio_file = SD.open(audio_filename, FILE_APPEND);
    if (audio_file)
    {  
      if (BITS_PER_SAMPLE == 16) // 16 bit default: appending original I2S chunks (e.g. 1024 values, 2048 bytes)
      { audio_file.write((uint8_t*)audio_buffer, bytes_read); }        
      if (BITS_PER_SAMPLE == 8)  // 8bit mode: appending calculated 1024 values instead (1024 bytes, bytes_read/2) 
